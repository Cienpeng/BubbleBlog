...
└── src/
    ...
    │       ├── lockouts.ts    # [新增] 登录尝试限制与锁定数据库查询
    │       └── captchas.ts    # [新增] 验证码生成记录与单次消费数据库查询
    │
    ├── markdown/
    ...
    └── services/
        ...
        └── captcha.ts         # [新增] 纯 JS 生成 SVG 验证码的逻辑服务
```
...
| `login_lockouts` | [新增] 登录封禁限制记录：id, ip, fingerprint, attempt_count, lockout_count, locked_until, updated_at |
| `captchas` | [新增] 一次性验证码防刷：id, code, expires_at |
...
└── src/
    ...
    └── components/
        ...
        │   ├── ImageCropperModal.tsx # [新增] 用于本地图片上传裁剪的核心弹窗组件
        │   └── SlidePanel.tsx  # 右侧抽屉滑入组件
        └── charts/
            ├── BarChart.tsx    # [修改] 修复在柱状图悬停时由于变换重叠导致 Enter/Leave 重复触发闪烁的 Bug
